    ...
    try {
      ...
      if (data) {
        setStandings(((data as Array<Record<string, unknown>> | null) ?? []).map((standingRow) => ({
          ...
          sets_for: Number(standingRow.sets_for ?? 0),
          sets_against: Number(standingRow.sets_against ?? 0),
          rally_points_for: Number(standingRow.rally_points_for ?? 0),
          rally_points_against: Number(standingRow.rally_points_against ?? 0),
