...
export function LinksPageView({ championships, publicLinkSections, loading = false }: LinksPageViewProps) {
  const [selectedChampionshipId, setSelectedChampionshipId] = useState<string>(ALL_PUBLIC_LINKS_FILTER_VALUE);
  const [selectedSeasonYear, setSelectedSeasonYear] = useState<string>(ALL_PUBLIC_LINKS_FILTER_VALUE);

  const availableChampionships = useMemo(() => {
    return resolveAvailablePublicLinkFilterChampionships(publicLinkSections, championships);
  }, [championships, publicLinkSections]);

  const availableSeasonYears = useMemo(() => {
    return resolveAvailablePublicLinkFilterYears(publicLinkSections, selectedChampionshipId);
  ...
  useEffect(() => {
    ...
    if (!availableChampionships.some((championship) => championship.id == selectedChampionshipId)) {
  ...
  useEffect(() => {
    ...
    if (!availableSeasonYears.some((seasonYear) => String(seasonYear) == selectedSeasonYear)) {
      ...
      <main className="container space-y-4 py-8">
        <section className="glass-panel p-4 animate-in fade-in-0 slide-in-from-bottom-2 duration-500">
          <div className="flex flex-col items-center justify-center gap-2">
            <h1 className="text-center text-2xl font-display font-bold">Links organizados por seção</h1>
            <p className="text-center text-sm text-muted-foreground">
              Consulte fotos, álbuns e materiais públicos por campeonato e ano, sem precisar navegar por URLs soltas.
            </p>
          </div>
        </section>
        ...
        <section className="glass-panel p-4 animate-in fade-in-0 slide-in-from-bottom-2 duration-500">
          <div className="grid gap-3 sm:grid-cols-2">
            <Select value={selectedChampionshipId} onValueChange={setSelectedChampionshipId}>
              ...
              <SelectContent>
                <SelectItem value={ALL_PUBLIC_LINKS_FILTER_VALUE}>Todos os campeonatos</SelectItem>
            ...
            <Select value={selectedSeasonYear} onValueChange={setSelectedSeasonYear}>
              ...
              <SelectContent>
                <SelectItem value={ALL_PUBLIC_LINKS_FILTER_VALUE}>Todos os anos</SelectItem>
        ...
        {loading ? (
          ...
          <section className="glass-panel p-8 text-center">
            <p className="font-display text-xl font-semibold">Nenhum link disponível com esse filtro</p>
            <p className="mt-2 text-sm text-muted-foreground">
              Ajuste o campeonato ou o ano para visualizar outras seções públicas.
            </p>
          </section>
        ) : (
              ...
              <article key={publicLinkSection.id} className="glass-panel p-5 animate-in fade-in-0 slide-in-from-bottom-2 duration-500">
                ...
                <div className="flex flex-wrap justify-center gap-3">
                  {(publicLinkSection.public_link_items ?? []).map((publicLinkItem) => (
                    ...
                    >
                      <a href={publicLinkItem.url} target="_blank" rel="noreferrer" className="block max-w-full">
                        <span className="flex max-w-full items-center gap-2">
                          <span className="min-w-0 truncate">{publicLinkItem.display_name}</span>
