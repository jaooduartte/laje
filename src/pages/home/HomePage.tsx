...
export function HomePage() {
  ...
  if (publicAccessLoading || authLoading || metricsLoading) {
    return (
      <div className="app-page">
        <Header />
        <main className="container flex min-h-[60vh] items-center justify-center py-8">
          <div className="flex items-center justify-center rounded-2xl border border-border/60 bg-background/50 px-8 py-10">
  ...
  const maintenanceItems = PUBLIC_PAGE_ACCESS_FIELD_ORDER
    .filter((field) => {
    ...
    .map((field) => ({
      ...
      reason: publicAccessSettings.blocked_message ?? "Manutenção preventiva ou ajuste operacional em andamento.",
    }));

  return <HomePageView items={homePageListItems} maintenanceItems={maintenanceItems} metrics={metrics} />;
}
