-- Renomeia variável local `round_number` para `current_round` em `generate_championship_knockout_for_competition`.
-- O label <<knockout_gen>> não era suficiente para resolver a ambiguidade entre a variável e
-- a coluna homónima em INSERTs, causando round_number = NULL (violando NOT NULL constraint).
...
BEGIN
  ...
  IF total_rounds > 1 THEN
    FOR current_round IN 2..total_rounds
    LOOP
      FOR slot_index IN 1..(bracket_size / (1 << current_round))
      LOOP
        SELECT
        ...
        WHERE bracket_matches_table.competition_id = _competition_id
          AND bracket_matches_table.phase = 'KNOCKOUT'::public.bracket_phase
          AND bracket_matches_table.is_third_place = false
          AND bracket_matches_table.round_number = current_round
          AND bracket_matches_table.slot_number = slot_index
        LIMIT 1;
        ...
        IF existing_match_id IS NULL THEN
          ...
          SET
            home_team_id = home_team_id,
            away_team_id = away_team_id,
            winner_team_id = desired_winner_team_id,
            is_bye = CASE
              WHEN home_team_id IS NULL AND away_team_id IS NULL THEN false
              WHEN home_team_id IS NOT NULL AND away_team_id IS NOT NULL THEN false
              ELSE true
