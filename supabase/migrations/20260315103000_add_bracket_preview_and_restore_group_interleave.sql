DO $migration_reapply_group_stage_interleave$
DECLARE
...
BEGIN
        ...
        CASE
          WHEN EXISTS (
            ...
            WHERE same_naipe_match_table.championship_id = _championship_id
              AND same_naipe_match_table.season_year = championship_current_season_year
  ...
  IF position('temp_pending_group_matches' IN function_definition) = 0
    ...
    RAISE EXCEPTION 'Não foi possível reaplicar a intercalação de jogos por chave na geração automática.';
  END IF;

  EXECUTE function_definition;
END;
$migration_reapply_group_stage_interleave$;

CREATE OR REPLACE FUNCTION public.preview_championship_bracket_groups(
...
BEGIN
  BEGIN
    PERFORM public.generate_championship_bracket_groups(_championship_id, COALESCE(_payload, '{}'::jsonb));
    RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = '__preview_success__';
  EXCEPTION
    WHEN SQLSTATE 'P0001' THEN
      IF SQLERRM = '__preview_success__' THEN
...
COMMENT ON FUNCTION public.preview_championship_bracket_groups(UUID, JSONB) IS 'Executa a validação completa da geração de chaves em subtransação com rollback forçado, retornando conflitos de agenda sem persistir dados.';
