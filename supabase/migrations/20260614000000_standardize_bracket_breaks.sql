-- Padroniza intervalos de dias para usar unicamente championship_bracket_day_breaks.
--
-- Problema: generate_championship_bracket_groups escrevia apenas nas colunas legadas
-- (championship_bracket_days.break_start_time/break_end_time). O editor de agenda
-- pós-criação já escrevia em championship_bracket_day_breaks. Dois caminhos divergentes.
--
-- Solução:
-- 1. Backfill conservador: preenche position=1 em championship_bracket_day_breaks
--    para dias que têm dados legados mas não têm entrada na posição 1 da nova tabela.
-- 2. Atualiza generate_championship_bracket_groups para também escrever em
--    championship_bracket_day_breaks logo após inserir championship_bracket_days.
-- 3. Remove o trigger sync_bracket_day_legacy_break_trigger (de 20260613030000) que
--    se torna redundante e pode causar interações desnecessárias.

-- 1. Backfill conservador
...
-- 2. Remove trigger redundante de 20260613030000 (se foi aplicado)
DROP TRIGGER IF EXISTS sync_bracket_day_legacy_break_trigger ON public.championship_bracket_days;
DROP FUNCTION IF EXISTS public.sync_bracket_day_legacy_break();

-- 3. Atualiza generate_championship_bracket_groups para escrever em
--    championship_bracket_day_breaks diretamente, sem depender de trigger.
...
DECLARE
  ...
  -- Variaveis para sincronizar intervalo com championship_bracket_day_breaks
...
BEGIN
  ...
  LOOP
    ...
    -- Sincroniza intervalo com championship_bracket_day_breaks (fonte única de verdade)
    v_day_break_start := CASE
      WHEN trim(COALESCE(schedule_day_record->>'break_start_time', '')) = '' THEN NULL
      ELSE (schedule_day_record->>'break_start_time')::time
    END;
    v_day_break_end := CASE
      WHEN trim(COALESCE(schedule_day_record->>'break_end_time', '')) = '' THEN NULL
      ELSE (schedule_day_record->>'break_end_time')::time
    END;

    IF v_day_break_start IS NOT NULL AND v_day_break_end IS NOT NULL THEN
      INSERT INTO public.championship_bracket_day_breaks (
        bracket_day_id, break_start_time, break_end_time, position
      ) VALUES (bracket_day_id, v_day_break_start, v_day_break_end, 1)
      ON CONFLICT (bracket_day_id, position) DO UPDATE SET
        break_start_time = EXCLUDED.break_start_time,
        break_end_time = EXCLUDED.break_end_time;
    ELSE
      DELETE FROM public.championship_bracket_day_breaks AS bd
      WHERE bd.bracket_day_id = bracket_day_id AND bd.position = 1;
    END IF;
