-- Alinha a geração do mata-mata com o novo modo "Só 1º por grupo (completa só se precisar)".
-- Regra esperada para qualifiers_per_group = 1:
--   - should_complete_knockout_with_best_second_placed_teams = true:
--       sempre expande para a próxima chave acima (ex.: 4 -> 8) e completa com melhores 2º.
--   - should_complete_knockout_with_best_second_placed_teams = false:
--       usa apenas melhores 1º quando a quantidade já fecha chave (potência de 2);
--       completa com melhores 2º apenas quando necessário para fechar a chave (ex.: 3 -> 4).
...
COMMENT ON FUNCTION public.generate_championship_knockout_for_competition(UUID, UUID, UUID) IS
  ...
  'Para qualifiers_per_group=1: '
  'se should_complete_knockout_with_best_second_placed_teams=true, sempre expande para a próxima chave e completa com melhores 2º; '
  'se false, usa só 1ºs quando já fecha chave e completa com melhores 2º apenas quando necessário.';
