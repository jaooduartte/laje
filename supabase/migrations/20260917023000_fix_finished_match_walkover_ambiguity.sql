...
BEGIN
  ...
  IF normalized_walkover_mode = 'NONE' OR normalized_walkover_mode = 'DOUBLE' THEN
    ...
    WHERE matches_table.id = _match_id;
  ELSE
    v_walkover_loser_team_id := CASE normalized_walkover_mode
      WHEN 'HOME_LOST' THEN match_record.home_team_id
      WHEN 'AWAY_LOST' THEN match_record.away_team_id
    END;

    winner_side := CASE
      WHEN v_walkover_loser_team_id = match_record.home_team_id THEN 'away'
      ELSE 'home'
    END;
...
COMMENT ON FUNCTION public.save_finished_match_walkover(UUID, TEXT)
IS 'Atualiza o W.O. de um jogo encerrado sem ambiguidade entre a variável PL/pgSQL do perdedor e a coluna matches.walkover_loser_team_id.';
