-- migration: 20260427160000_fix_second_place_seed_order_for_qualifiers_per_group_2.sql
--
-- Problema:
--   Para competições com qualifiers_per_group=2 (ex: Beach Soccer Feminino),
--   os 2ºs colocados eram ordenados por pool_rank (melhor saldo/pontos entre grupos),
--   fazendo o array de seeds ficar [1ºG1, 1ºG2, 2ºG2*, 2ºG1*] quando o 2º do G2
--   tinha stats melhores que o 2º do G1.
--
--   Com cross seeding clássico [1,4,2,3] isso produzia:
--     slot1 = seed1(1ºG1) × seed4(2ºG1)   ← errado
--     slot2 = seed2(1ºG2) × seed3(2ºG2)   ← errado
--
-- Correção:
--   O pool_rank só é relevante no cenário "melhores 2ºs"
--   (qualifiers_per_group=1 E should_complete_knockout_with_best_second_placed_teams=true).
--   Para qualifiers_per_group=2, os qualificados devem ser ordenados por group_number,
--   dando [1ºG1, 1ºG2, 2ºG1, 2ºG2] → cross seeding [1,4,2,3]:
--     slot1 = seed1(1ºG1) × seed4(2ºG2)   ← correto
--     slot2 = seed2(1ºG2) × seed3(2ºG1)   ← correto
...
BEGIN
  ...
  FOR ranking_record IN
    ...
    ORDER BY
      ...
      -- Para o cenário de "melhores 2ºs" (qualifiers_per_group=1 + best_seconds),
      -- usa pool_rank para ordenar os classificados pelo melhor desempenho cross-grupo.
      -- Para qualifiers_per_group > 1 (ex: 1º e 2º de cada grupo), ordena pelo
      -- número do grupo para manter a sequência G1→G2→G3 sem distorção por stats.
      CASE
...
COMMENT ON FUNCTION public.generate_championship_knockout_for_competition(UUID, UUID, UUID) IS
  'Gera o mata-mata por competição com seeding cruzado clássico. '
  ...
  'Usa count(match_id) para is_group_finished, ignorando slots de grupo sem jogo associado.';

-- Regera todos os KOs com a lógica corrigida.
DO $regenerate$
DECLARE
  competition_record RECORD;
BEGIN
