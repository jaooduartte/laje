...
BEGIN
  ...
  FOR pending_group_match_record IN
    SELECT
      ...
      pending_group_matches_table.sport_id,
      pending_group_matches_table.division,
    ...
    ORDER BY
      ...
      pending_group_matches_table.competition_priority ASC,
      pending_group_matches_table.competition_slot_number ASC
  LOOP
    competition_id := pending_group_match_record.competition_id;
    sport_id := pending_group_match_record.sport_id;
    division_value := pending_group_match_record.division;
...
COMMENT ON FUNCTION public.validate_match_conflict() IS 'Valida a fila diária da partida, horários reais e bloqueia novos jogos ao vivo quando a capacidade de quadras compatíveis do dia foi atingida.';
COMMENT ON FUNCTION public.generate_championship_bracket_groups(UUID, JSONB) IS 'Cria edição de chaveamento por temporada e gera a fase de grupos em fila diária, intercalando modalidades e naipes por chave para distribuição operacional.';
