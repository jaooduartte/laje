...
BEGIN
  ...
  FROM (
    SELECT s.team_id, t.name AS team_name,
      (
        SUM(s.points)
        + COALESCE(MAX(bc.total_adjustment), 0)
        + COALESCE(MAX(ob.total_opening_bonus), 0)
    ...
    LEFT JOIN (
      SELECT adjustments_table.team_id, SUM(adjustments_table.points) AS total_opening_bonus
      FROM public.championship_overall_score_adjustments AS adjustments_table
      WHERE adjustments_table.championship_id = ANY(selected_championship_ids)
        AND adjustments_table.adjustment_type = 'OPENING_CEREMONY'
      GROUP BY adjustments_table.team_id
    ) AS ob ON ob.team_id = s.team_id
  ...
  FROM (
    ...
    LEFT JOIN (
      ...
      FROM public.championship_overall_score_adjustments AS adjustments_table
      WHERE adjustments_table.championship_id = ANY(selected_championship_ids)
        AND adjustments_table.season_year = resolved_season_year
        AND adjustments_table.adjustment_type = 'OPENING_CEREMONY'
      GROUP BY adjustments_table.team_id
    ) AS ob ON ob.team_id = s.team_id
  ...
  biggest_margin_with_team AS (
    SELECT
      ...
      CASE
        WHEN championship_sports_table.result_rule = 'SETS' THEN 'sets de diferença'
        WHEN sports_table.name ILIKE '%basquet%' THEN 'pontos de diferença'
        ELSE 'gols de diferença'
      END AS margin_unit
    FROM biggest_margin
    INNER JOIN public.teams AS teams ON teams.id = biggest_margin.team_id
    INNER JOIN public.sports AS sports_table ON sports_table.id = biggest_margin.sport_id
    LEFT JOIN public.championship_sports AS championship_sports_table
      ON championship_sports_table.championship_id = biggest_margin.championship_id
      AND championship_sports_table.sport_id = biggest_margin.sport_id
  ...
  SELECT COALESCE(
    jsonb_build_array(
      ...
      jsonb_build_object(
        ...
        'unit', (SELECT margin_unit FROM biggest_margin_with_team LIMIT 1),
        ...
        'sport_name', (SELECT sport_name FROM biggest_margin_with_team LIMIT 1)
      ),
      jsonb_build_object(
        'id', 'MOST_PODIUMS',
