-- migration: 20260427170000_fix_ko_schedule_order_by_seed_rank.sql
--
-- Problema:
--   O seeding clássico balanceado [1,8,4,5,2,7,3,6] colocava seeds 1 e 2 em lados
--   OPOSTOS da chave, fazendo slots 1 e 2 gerarem (1v8) e (4v5) no mesmo lado — o
--   que não reflete o formato da planilha oficial, onde os dois primeiros jogos
--   são (1v8) e (2v7) no mesmo lado (o vencedor de cada um se encontra na semi).
--
-- Correção:
--   Trocar o algoritmo recursivo de seeding balanceado pelo seeding LINEAR:
--     slot k: seed k × seed (bracket_size+1-k)
--   Para chave de 8: [1,8, 2,7, 3,6, 4,5]
--     slot1=1v8, slot2=2v7  → mesmo lado (SF1)
--     slot3=3v6, slot4=4v5  → mesmo lado (SF2)
--   Para chave de 4: [1,4, 2,3] — idêntico ao anterior.
--   Para chave de 2: [1,2]     — sem mudança.
--
--   Com seeding linear, slot_index já está na ordem de seed rank, então o
--   agendamento (create_championship_knockout_match_schedule) pode ser feito
--   diretamente no loop de slot_index, sem passo adicional de reordenação.
...
BEGIN
  ...
  -- Constrói a ordem de seeds LINEAR: slot k pareia seed k vs seed (N+1-k).
  -- bracket_size=8 → [1,8, 2,7, 3,6, 4,5]
  --   slot1=1v8, slot2=2v7 → mesmo lado (SF1)
  --   slot3=3v6, slot4=4v5 → mesmo lado (SF2)
  -- bracket_size=4 → [1,4, 2,3] — idêntico ao seeding balanceado.
  ...
  IF existing_knockout_count = 0 THEN
    ...
    LOOP
      ...
      -- Com seeding linear, slot_index já está em ordem de seed rank,
      -- então o agendamento direto por slot produz a ordem correta: 1v8, 2v7, 3v6, 4v5.
...
COMMENT ON FUNCTION public.generate_championship_knockout_for_competition(UUID, UUID, UUID) IS
  ...
  'Para chave de 8: [1v8, 2v7, 3v6, 4v5] — seeds 1 e 2 no mesmo lado (SF1). '
