-- migration: 20260427180000_fix_ko_seeding_linear_order.sql
--
-- Problema:
--   O seeding balanceado clássico [1,8,4,5,2,7,3,6] colocava seeds 1 e 2 em lados
--   OPOSTOS da chave. Com isso, slot1=1v8 e slot2=4v5 ficavam no mesmo lado (SF1),
--   fazendo seeds 1 e 4 se encontrarem na semi — não é o comportamento esperado.
--
-- Correção:
--   Trocar o algoritmo recursivo pelo seeding LINEAR: slot k = seed_k × seed_(N+1-k).
--   Para chave de 8: [1,8, 2,7, 3,6, 4,5]
--     slot1=1v8, slot2=2v7 → mesmo lado (SF1) ← seeds 1 e 2 se encontram na semi
--     slot3=3v6, slot4=4v5 → mesmo lado (SF2)
--   Para chave de 4: [1,4, 2,3] — resultado idêntico ao anterior.
--   Para chave de 2: [1,2]     — sem mudança.
--
--   Com seeding linear, slot_index já está em ordem de seed rank, portanto o
--   agendamento (queue_position) em ordem de slot produz naturalmente: 1v8, 2v7, 3v6, 4v5.
...
BEGIN
  ...
  --   slot1=1v8 e slot2=2v7 no mesmo lado (SF1) → seeds 1 e 2 se encontram na semi
  --   slot3=3v6 e slot4=4v5 no mesmo lado (SF2)
  -- bracket_size=4 → [1,4, 2,3] — mesmo resultado do seeding anterior
