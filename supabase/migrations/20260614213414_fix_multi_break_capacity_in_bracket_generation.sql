-- Corrige o cálculo de capacidade diária na geração da fase de grupos para
-- considerar todos os intervalos salvos em championship_bracket_day_breaks.
--
-- Sem isso, a geração inicial desconta apenas o intervalo legado único em
-- championship_bracket_days e pode manter jogos em slots que já não cabem mais
-- no dia quando existem múltiplos intervalos.
  ...
  FOR affected_bracket_edition_id IN
    ...
    day_capacities AS (
      SELECT
        ...
        d.event_date,
        CASE
          WHEN d.event_date = ld.last_event_date THEN NULL
          ELSE GREATEST(1, FLOOR(
            (
              EXTRACT(EPOCH FROM d.end_time) / 60.0
              - EXTRACT(EPOCH FROM d.start_time) / 60.0
            ...
            / GREATEST(1, COALESCE((
                SELECT MIN(public.resolve_championship_sport_duration_minutes(cbe.championship_id, cs.sport_id))
    ...
    JOIN day_capacities AS dc
      ON dc.bracket_edition_id = cbm.bracket_edition_id
      AND dc.event_date = m.scheduled_date
    WHERE m.status = 'SCHEDULED'::public.match_status
      AND m.scheduled_slot IS NOT NULL
      AND dc.max_slot_count IS NOT NULL
      AND m.scheduled_slot > dc.max_slot_count
  LOOP
