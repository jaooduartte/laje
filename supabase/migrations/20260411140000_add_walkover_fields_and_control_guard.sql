ALTER TABLE public.matches
  ADD COLUMN IF NOT EXISTS is_walkover BOOLEAN;

UPDATE public.matches
SET is_walkover = false
WHERE is_walkover IS NULL;

ALTER TABLE public.matches
  ALTER COLUMN is_walkover SET DEFAULT false,
  ALTER COLUMN is_walkover SET NOT NULL;

COMMENT ON COLUMN public.matches.is_walkover IS 'Indica se a partida foi encerrada por W.O.';

ALTER TABLE public.matches
  ADD COLUMN IF NOT EXISTS walkover_loser_team_id UUID NULL;

COMMENT ON COLUMN public.matches.walkover_loser_team_id IS 'Atlética que perdeu por W.O. na partida.';
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'matches_walkover_loser_team_id_fkey'
  ) THEN
    ALTER TABLE public.matches
      ADD CONSTRAINT matches_walkover_loser_team_id_fkey
      FOREIGN KEY (walkover_loser_team_id)
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'matches_walkover_consistency_check'
  ) THEN
    ALTER TABLE public.matches
