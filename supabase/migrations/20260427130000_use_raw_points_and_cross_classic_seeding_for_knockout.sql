-- migration: 20260427130000_use_raw_points_and_cross_classic_seeding_for_knockout.sql
--
-- 1. Remove o fator de correção (pontos × max_jogos / jogos_do_grupo) do ranking
--    do pool de qualificação. O pool agora ordena por pontuação bruta:
--    Pontos → Vitórias → Saldo → Pró → desempate manual → nome.
-- 2. Substitui o pareamento sequencial da primeira rodada (1×2, 3×4, ...) pelo
--    pareamento cruzado clássico (chave balanceada: 1×8, 4×5, 2×7, 3×6), de modo
--    que o vencedor do seed #1 só possa enfrentar o seed #2 na final.
-- 3. Apaga todo o mata-mata existente e regera com a nova lógica para cada
--    bracket_edition. Matches de KO já finalizados terão os placares descartados
--    e precisarão ser relançados manualmente.
...
COMMENT ON FUNCTION public.get_championship_bracket_competition_qualification_pool_rankings(UUID, UUID) IS
  'Ranking do pool cross-grupo para seeding do mata-mata. Usa pontos brutos (sem fator de correção): Pontos → Vitórias → Saldo → Pró → desempate manual → nome.';
...
BEGIN
  ...
  -- Ex.: bracket_size = 8 → [1, 8, 4, 5, 2, 7, 3, 6].
  -- Regra recursiva: dado seeds de tamanho k, para tamanho 2k emite, para cada
  -- s em seeds, a sequência s, (2k + 1 - s).
...
COMMENT ON FUNCTION public.generate_championship_knockout_for_competition(UUID, UUID, UUID) IS
  'Gera o mata-mata por competição usando seeding cruzado clássico (1×N, 4×5, 2×N-1, 3×N-2, ...) com o ranking do pool de qualificação (pontos brutos).';

-- Regera todo o mata-mata existente com a nova lógica. Matches de KO previamente
-- lançados serão apagados; a fase de grupos é preservada.
DO $regenerate$
DECLARE
  competition_record RECORD;
BEGIN
  ...
  LOOP
    -- Remove primeiro os matches (ON DELETE SET NULL no bracket_matches.match_id
    -- garante que a FK fica NULL antes do delete do bracket).
    DELETE FROM public.matches
    WHERE id IN (
      SELECT bracket_matches_table.match_id
      FROM public.championship_bracket_matches AS bracket_matches_table
      WHERE bracket_matches_table.competition_id = competition_record.competition_id
        AND bracket_matches_table.phase = 'KNOCKOUT'::public.bracket_phase
        AND bracket_matches_table.match_id IS NOT NULL
    );

    -- Remove as referências source_* / next_* antes de apagar, evitando violação
    -- de FK self-referenciada.
