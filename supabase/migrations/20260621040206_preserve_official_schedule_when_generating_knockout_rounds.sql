-- Reaplica a numeração oficial da planilha sem recriar confrontos nem chamar
-- redistribuição automática. O objetivo aqui é só corrigir a edição afetada
-- após a geração das semifinais já existentes.
...
BEGIN
  ...
  INSERT INTO tmp_schedule_repair_rows (
    scheduled_date,
    location,
    court_name,
    planned_start_time,
  ...
  CREATE TEMP TABLE tmp_schedule_repair_group_stage_matches AS
  SELECT
    repair_rows.*,
    matches_table.id AS match_id
  FROM tmp_schedule_repair_rows AS repair_rows
  JOIN public.matches AS matches_table
    ...
    AND matches_table.naipe = repair_rows.naipe
    AND matches_table.division IS NOT DISTINCT FROM repair_rows.division
  ...
  JOIN public.teams AS home_teams_table
    ...
    AND public.normalize_bracket_entity_name(home_teams_table.name) = public.normalize_bracket_entity_name(repair_rows.home_team_name)
  JOIN public.teams AS away_teams_table
    ON away_teams_table.id = matches_table.away_team_id
    AND public.normalize_bracket_entity_name(away_teams_table.name) = public.normalize_bracket_entity_name(repair_rows.away_team_name)
  ...
  FROM tmp_schedule_repair_group_stage_matches;

  IF matched_group_stage_match_count <> expected_group_stage_match_count THEN
    RAISE EXCEPTION
      'A migration esperava mapear % jogos da fase de grupos, mas encontrou %.',
  ...
  SET
    scheduled_slot = NULL,
    queue_position = NULL
  FROM tmp_schedule_repair_group_stage_matches AS group_stage_matches_table
  ...
  GET DIAGNOSTICS updated_group_stage_match_count = ROW_COUNT;
  ...
  USING public.championship_bracket_matches AS bracket_matches_table
  WHERE matches_table.id = bracket_matches_table.match_id
    AND bracket_matches_table.competition_id = feminino_acesso_competition_id
  ...
  IF (
    ...
    RAISE EXCEPTION 'As duas semifinais de Feminino B precisam existir antes deste realinhamento.';
  END IF;
  ...
  GET DIAGNOSTICS updated_feminino_acesso_semifinal_count = ROW_COUNT;

  IF updated_feminino_acesso_semifinal_count <> 2 THEN
    RAISE EXCEPTION
      'A migration esperava reposicionar 2 semifinais do Feminino B, mas atualizou %.',
      updated_feminino_acesso_semifinal_count;
  END IF;
