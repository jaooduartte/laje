...
BEGIN
  ...
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        ...
        'team_id', ranking.team_id,
        'team_name', ranking.team_name,
        'titles_count', ranking.total_points
      )
      ORDER BY ranking.championship_order ASC
    ),
    '[]'::jsonb
  )
  ...
  FROM (
    SELECT DISTINCT ON (championships.code)
      ...
      CASE championships.code
        WHEN 'CLV'::public.championship_code THEN 1
        WHEN 'SOCIETY'::public.championship_code THEN 2
        WHEN 'INTERLAJE'::public.championship_code THEN 3
        ELSE 99
      END AS championship_order,
      standings.team_id,
      teams.name AS team_name,
      SUM(standings.points)::int AS total_points
    FROM public.standings AS standings
  ...
  FROM (
    ...
    INNER JOIN public.teams AS teams
      ON teams.id = standings.team_id
    WHERE standings.championship_id = ANY(selected_championship_ids)
      AND standings.season_year = resolved_season_year
    GROUP BY standings.team_id, teams.name
  ...
  biggest_margin AS (
    SELECT
      CASE
        ...
        ELSE matches.away_team_id
      END AS winner_team_id,
      ABS(matches.home_score - matches.away_score)::int AS margin
    FROM public.matches AS matches
    WHERE matches.season_year = resolved_season_year
      AND matches.championship_id = ANY(selected_championship_ids)
      AND matches.status = 'FINISHED'::public.match_status
  ),
  biggest_margin_with_team AS (
    SELECT
      biggest_margin.winner_team_id AS team_id,
      teams.name AS team_name,
      biggest_margin.margin
    FROM biggest_margin
    INNER JOIN public.teams AS teams ON teams.id = biggest_margin.winner_team_id
    ORDER BY biggest_margin.margin DESC, teams.name ASC
    LIMIT 1
  ),
  modalities_year AS (
    SELECT
  ...
  next_events AS (
    SELECT
      ...
      CASE
        WHEN league_events.organizer_type = 'LAJE'::public.league_event_organizer_type THEN 'LAJE'
        ELSE COALESCE(teams.name, 'Atlética')
  ...
  SELECT CASE
    WHEN (SELECT event_date FROM next_day) IS NULL THEN NULL
        ...
        (
          SELECT jsonb_agg(
            jsonb_build_object(
              ...
              'event_type', next_events.event_type,
              'organizer_name', next_events.organizer_name
            )
          )
          FROM next_events
        ),
