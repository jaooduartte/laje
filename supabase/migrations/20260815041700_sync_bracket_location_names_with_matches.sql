...
BEGIN
  ...
  WITH current_court_sports AS (
  SELECT
    days_table.bracket_edition_id,
    public.normalize_bracket_entity_name(locations_table.name) AS location_name,
    courts_table.name AS court_name,
    array_agg(DISTINCT court_sports_table.sport_id ORDER BY court_sports_table.sport_id) AS sport_ids
  FROM public.championship_bracket_days AS days_table
  JOIN public.championship_bracket_locations AS locations_table
...
match_court_sports AS (
  SELECT
    bracket_matches_table.bracket_edition_id,
    public.normalize_bracket_entity_name(matches_table.location) AS location_name,
    public.normalize_bracket_entity_name(matches_table.court_name) AS previous_court_name,
    array_agg(DISTINCT matches_table.sport_id ORDER BY matches_table.sport_id) AS sport_ids
  ...
  WHERE matches_table.location IS NOT NULL
    AND matches_table.court_name IS NOT NULL
    AND matches_table.sport_id IS NOT NULL
  GROUP BY
    bracket_matches_table.bracket_edition_id,
    public.normalize_bracket_entity_name(matches_table.location),
    public.normalize_bracket_entity_name(matches_table.court_name)
),
unique_court_name_repairs AS (
  SELECT
    match_court_sports.bracket_edition_id,
    match_court_sports.location_name,
    match_court_sports.previous_court_name,
    min(current_court_sports.court_name) AS court_name
  FROM match_court_sports
  JOIN current_court_sports
    ON current_court_sports.bracket_edition_id = match_court_sports.bracket_edition_id
    AND current_court_sports.location_name = match_court_sports.location_name
    AND current_court_sports.sport_ids = match_court_sports.sport_ids
  GROUP BY
    match_court_sports.bracket_edition_id,
    match_court_sports.location_name,
    match_court_sports.previous_court_name
  HAVING count(DISTINCT current_court_sports.court_name) = 1
)
  UPDATE public.matches AS matches_table
  SET court_name = unique_court_name_repairs.court_name
  FROM public.championship_bracket_matches AS bracket_matches_table,
    unique_court_name_repairs
  WHERE bracket_matches_table.match_id = matches_table.id
    AND unique_court_name_repairs.bracket_edition_id = bracket_matches_table.bracket_edition_id
    AND unique_court_name_repairs.location_name = public.normalize_bracket_entity_name(matches_table.location)
    AND unique_court_name_repairs.previous_court_name = public.normalize_bracket_entity_name(matches_table.court_name)
    AND matches_table.court_name IS DISTINCT FROM unique_court_name_repairs.court_name;
