CREATE TABLE championship_bracket_preview_private.job_events (
  ...
  group_match_id UUID NULL REFERENCES championship_bracket_preview_private.matches(id) ON DELETE SET NULL,
  knockout_match_id UUID NULL REFERENCES championship_bracket_preview_private.knockout_matches(id) ON DELETE SET NULL,
  details JSONB NOT NULL DEFAULT '{}'::jsonb,
  occurred_at TIMESTAMPTZ NOT NULL DEFAULT clock_timestamp(),
  UNIQUE NULLS NOT DISTINCT (job_id, event_type, group_match_id, knockout_match_id)
);

CREATE INDEX championship_bracket_preview_job_events_history_idx
  ON championship_bracket_preview_private.job_events (job_id, occurred_at, id);

ALTER TABLE championship_bracket_preview_private.job_events ENABLE ROW LEVEL SECURITY;
...
CREATE TRIGGER championship_bracket_preview_group_match_scheduled_event_trigger
AFTER UPDATE OF assigned
ON championship_bracket_preview_private.matches
FOR EACH ROW
EXECUTE FUNCTION championship_bracket_preview_private.record_group_match_scheduled_event();
...
CREATE TRIGGER championship_bracket_preview_knockout_match_scheduled_event_trigger
AFTER UPDATE OF scheduled_date
ON championship_bracket_preview_private.knockout_matches
FOR EACH ROW
EXECUTE FUNCTION championship_bracket_preview_private.record_knockout_match_scheduled_event();
...
REVOKE ALL ON championship_bracket_preview_private.job_events
FROM PUBLIC, anon, authenticated;

REVOKE ALL ON FUNCTION championship_bracket_preview_private.record_group_match_scheduled_event()
FROM PUBLIC, anon, authenticated;

REVOKE ALL ON FUNCTION championship_bracket_preview_private.record_knockout_match_scheduled_event()
FROM PUBLIC, anon, authenticated;
