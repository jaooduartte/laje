-- Substitui integralmente a versão da 20260612000000 (aplicar depois dela).
-- Adiciona suporte a preferências de quadra por naipe/divisão (semântica preferencial):
--   1. Persiste courts[].sport_priorities do payload em championship_bracket_court_sports
--      (preferred_naipe / preferred_division), mantendo compat com courts[].sport_ids.
--   2. Na alocação de slots, limita jogos por (modalidade, naipe) e (modalidade, divisão)
--      às quadras que aceitam o tipo (preferem-no ou não têm preferência), com fallback:
--      quando não restam jogos pendentes de outros tipos no dia, libera o total de quadras.
--      Naipe e divisão são eixos independentes; a combinação de ambos na mesma quadra é
--      tratada de forma aproximada (cada eixo é verificado isoladamente).
...
DECLARE
  ...
  max_slots_for_day INTEGER;
  current_sport_naipe_court_count INTEGER;
  current_sport_naipe_slot_match_count INTEGER;
  current_sport_naipe_has_preference BOOLEAN;
  has_other_naipe_pending_today BOOLEAN;
  current_sport_division_court_count INTEGER;
  current_sport_division_slot_match_count INTEGER;
  current_sport_division_has_preference BOOLEAN;
  has_other_division_pending_today BOOLEAN;
BEGIN
  ...
  -- Reaproveitamento de rascunho existente (20260505193000)
  ...
  LOOP
    ...
    LOOP
      ...
      LOOP
        ...
        -- Preferências de naipe/divisão por quadra+modalidade (opcional no payload)
        IF jsonb_typeof(COALESCE(court_record->'sport_priorities', 'null'::jsonb)) = 'array' THEN
          FOR court_sport_record IN
            SELECT value
            FROM jsonb_array_elements(court_record->'sport_priorities')
          LOOP
            INSERT INTO public.championship_bracket_court_sports (
              bracket_court_id,
              sport_id,
              ...
              bracket_court_id,
              (court_sport_record->>'sport_id')::uuid,
              NULLIF(trim(COALESCE(court_sport_record->>'preferred_naipe', '')), '')::public.match_naipe,
              NULLIF(trim(COALESCE(court_sport_record->>'preferred_division', '')), '')::public.team_division
            )
            ON CONFLICT ON CONSTRAINT championship_bracket_court_sports_upsert_unique
            DO UPDATE SET
              preferred_naipe = EXCLUDED.preferred_naipe,
  ...
  -- Capacidade preferencial por (dia, modalidade, naipe): quadras que aceitam o naipe
  -- (preferem-no ou não têm preferência). has_preference indica se alguma quadra da
  -- modalidade no dia tem preferência de naipe configurada.
  DROP TABLE IF EXISTS temp_day_sport_naipe_court_counts;

  CREATE TEMP TABLE temp_day_sport_naipe_court_counts (
    event_date DATE NOT NULL,
    sport_identity TEXT NOT NULL,
    naipe public.match_naipe NOT NULL,
    court_count INTEGER NOT NULL,
    has_preference BOOLEAN NOT NULL,
    PRIMARY KEY (event_date, sport_identity, naipe)
  ) ON COMMIT DROP;

  INSERT INTO temp_day_sport_naipe_court_counts (event_date, sport_identity, naipe, court_count, has_preference)
  SELECT
    days_table.event_date,
    regexp_replace(lower(trim(COALESCE(sports_table.name, court_sports_table.sport_id::text))), '[^a-z0-9]+', '', 'g') AS sport_identity,
    naipe_values.naipe,
    COUNT(DISTINCT courts_table.id) FILTER (
      WHERE court_sports_table.preferred_naipe IS NULL
        OR court_sports_table.preferred_naipe = naipe_values.naipe
    )::integer AS court_count,
    bool_or(court_sports_table.preferred_naipe IS NOT NULL) AS has_preference
  FROM public.championship_bracket_days AS days_table
  JOIN public.championship_bracket_locations AS locations_table
    ON locations_table.bracket_day_id = days_table.id
  JOIN public.championship_bracket_courts AS courts_table
  ...
  -- Capacidade preferencial por (dia, modalidade, divisão), análoga à de naipe.
  DROP TABLE IF EXISTS temp_day_sport_division_court_counts;

  CREATE TEMP TABLE temp_day_sport_division_court_counts (
    event_date DATE NOT NULL,
    sport_identity TEXT NOT NULL,
    division public.team_division NOT NULL,
    court_count INTEGER NOT NULL,
    has_preference BOOLEAN NOT NULL,
    PRIMARY KEY (event_date, sport_identity, division)
  ) ON COMMIT DROP;

  INSERT INTO temp_day_sport_division_court_counts (event_date, sport_identity, division, court_count, has_preference)
  SELECT
    days_table.event_date,
    regexp_replace(lower(trim(COALESCE(sports_table.name, court_sports_table.sport_id::text))), '[^a-z0-9]+', '', 'g') AS sport_identity,
    division_values.division,
    COUNT(DISTINCT courts_table.id) FILTER (
      WHERE court_sports_table.preferred_division IS NULL
        OR court_sports_table.preferred_division = division_values.division
    )::integer AS court_count,
    bool_or(court_sports_table.preferred_division IS NOT NULL) AS has_preference
  FROM public.championship_bracket_days AS days_table
  JOIN public.championship_bracket_locations AS locations_table
    ON locations_table.bracket_day_id = days_table.id
  JOIN public.championship_bracket_courts AS courts_table
  ...
  -- Capacidade máxima de slots por dia: floor((janela_min - intervalo_min) / duracao_min_esporte)
  DROP TABLE IF EXISTS temp_day_max_slot_counts;

  CREATE TEMP TABLE temp_day_max_slot_counts (
    event_date DATE PRIMARY KEY,
    max_slot_count INTEGER NOT NULL
  ) ON COMMIT DROP;

  INSERT INTO temp_day_max_slot_counts (event_date, max_slot_count)
  SELECT
    d.event_date,
    CASE
      -- Último dia não tem limite: jogos podem ultrapassar o horário sem impedir a geração
      WHEN d.event_date = (
        SELECT MAX(d2.event_date)
        FROM championship_bracket_days d2
        WHERE d2.bracket_edition_id = bracket_edition_id
      ) THEN 2147483647
      ELSE GREATEST(1, FLOOR(
        ...
        / GREATEST(1, COALESCE((
            ...
            FROM championship_bracket_locations l
            JOIN championship_bracket_courts c ON c.bracket_location_id = l.id
            JOIN championship_bracket_court_sports cs ON cs.bracket_court_id = c.id
            WHERE l.bracket_day_id = d.id
          ), 35))
      ))::INTEGER
    END AS max_slot_count
  FROM championship_bracket_days d
  ...
  WHILE EXISTS (SELECT 1 FROM temp_ordered_group_matches) LOOP
    ...
    LOOP
      -- Capacidade esgotada: mover jogos restantes deste dia para o próximo disponível por modalidade
      IF candidate_queue_position > max_slots_for_day THEN
        UPDATE temp_ordered_group_matches AS tgm
        SET scheduled_date = next_day.next_event_date
        FROM (
          SELECT tgm2.sport_id, MIN(d.event_date) AS next_event_date
          FROM temp_ordered_group_matches tgm2
          JOIN championship_bracket_days d
            ON d.bracket_edition_id = bracket_edition_id
            AND d.event_date > selected_queue_date
          JOIN championship_bracket_locations l ON l.bracket_day_id = d.id
          JOIN championship_bracket_courts c ON c.bracket_location_id = l.id
          JOIN championship_bracket_court_sports cs ON cs.bracket_court_id = c.id
            AND cs.sport_id = tgm2.sport_id
          WHERE tgm2.scheduled_date = selected_queue_date
          GROUP BY tgm2.sport_id
        ) AS next_day
        WHERE tgm.scheduled_date = selected_queue_date
          AND tgm.sport_id = next_day.sport_id;

        IF EXISTS (SELECT 1 FROM temp_ordered_group_matches WHERE scheduled_date = selected_queue_date) THEN
          RAISE EXCEPTION 'Dias insuficientes na agenda: adicione mais dias de campeonato para acomodar todos os jogos.';
        END IF;
      ...
      LOOP
        ...
        -- Capacidade preferencial por NAIPE: enquanto houver jogos pendentes de outros
        -- naipes desta modalidade no dia, reserva as quadras preferidas deles.
        SELECT naipe_courts_table.court_count, naipe_courts_table.has_preference
        INTO current_sport_naipe_court_count, current_sport_naipe_has_preference
        FROM temp_day_sport_naipe_court_counts AS naipe_courts_table
        WHERE naipe_courts_table.event_date = selected_queue_date
          AND naipe_courts_table.sport_identity = pending_sport_identity
          AND naipe_courts_table.naipe = pending_group_match_record.naipe;

        IF COALESCE(current_sport_naipe_has_preference, false)
          AND COALESCE(current_sport_naipe_court_count, 0) < current_sport_slot_count THEN
          SELECT COUNT(*)
          INTO current_sport_naipe_slot_match_count
          FROM temp_assigned_queue_slots AS assigned_slots_table
          WHERE assigned_slots_table.scheduled_date = selected_queue_date
            AND assigned_slots_table.scheduled_slot = candidate_queue_position
            AND assigned_slots_table.sport_identity = pending_sport_identity
          ...
          IF current_sport_naipe_slot_match_count >= COALESCE(current_sport_naipe_court_count, 0) THEN
            -- Só adia se o outro naipe pendente tem quadra que o aceita (court_count > 0),
            -- evitando impasse quando dois naipes sem capacidade se esperariam mutuamente.
            has_other_naipe_pending_today := EXISTS (
              SELECT 1
              FROM temp_ordered_group_matches AS other_matches_table
              JOIN temp_day_sport_naipe_court_counts AS other_naipe_courts_table
                ON other_naipe_courts_table.event_date = selected_queue_date
                AND other_naipe_courts_table.sport_identity = other_matches_table.sport_identity
                AND other_naipe_courts_table.naipe = other_matches_table.naipe
                AND other_naipe_courts_table.court_count > 0
              WHERE other_matches_table.scheduled_date = selected_queue_date
                AND other_matches_table.sport_identity = pending_sport_identity
                AND other_matches_table.naipe != pending_group_match_record.naipe
            );

            IF has_other_naipe_pending_today THEN
              CONTINUE;
            END IF;
            -- fallback preferencial: sem outros naipes pendentes, libera o total de quadras
          END IF;
        END IF;

        -- Capacidade preferencial por DIVISÃO (apenas quando o jogo tem divisão), análoga.
        IF pending_group_match_record.division IS NOT NULL THEN
          SELECT division_courts_table.court_count, division_courts_table.has_preference
          INTO current_sport_division_court_count, current_sport_division_has_preference
          FROM temp_day_sport_division_court_counts AS division_courts_table
          WHERE division_courts_table.event_date = selected_queue_date
            AND division_courts_table.sport_identity = pending_sport_identity
            AND division_courts_table.division = pending_group_match_record.division;

          IF COALESCE(current_sport_division_has_preference, false)
            AND COALESCE(current_sport_division_court_count, 0) < current_sport_slot_count THEN
            SELECT COUNT(*)
            INTO current_sport_division_slot_match_count
            FROM temp_assigned_queue_slots AS assigned_slots_table
            WHERE assigned_slots_table.scheduled_date = selected_queue_date
              AND assigned_slots_table.scheduled_slot = candidate_queue_position
              AND assigned_slots_table.sport_identity = pending_sport_identity
              AND assigned_slots_table.division = pending_group_match_record.division;

            IF current_sport_division_slot_match_count >= COALESCE(current_sport_division_court_count, 0) THEN
              has_other_division_pending_today := EXISTS (
                SELECT 1
                FROM temp_ordered_group_matches AS other_matches_table
                JOIN temp_day_sport_division_court_counts AS other_division_courts_table
                  ON other_division_courts_table.event_date = selected_queue_date
                  AND other_division_courts_table.sport_identity = other_matches_table.sport_identity
                  AND other_division_courts_table.division = other_matches_table.division
                  AND other_division_courts_table.court_count > 0
                WHERE other_matches_table.scheduled_date = selected_queue_date
                  AND other_matches_table.sport_identity = pending_sport_identity
                  AND other_matches_table.division IS NOT NULL
                  AND other_matches_table.division != pending_group_match_record.division
              );

              IF has_other_division_pending_today THEN
                CONTINUE;
              END IF;
            END IF;
          END IF;
